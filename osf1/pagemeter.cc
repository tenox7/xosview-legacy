...
//  vm_statistics() counts pages, not bytes, and its pagein counter includes
//  file pages faulted in through the unified buffer cache as well as traffic
//  to and from swap.
...
void PageMeter::getpageinfo( void ){
  if ( !OSF1Stats::paging( pageinfo_[pageindex_][0], pageinfo_[pageindex_][1] ) )
    return;
