...
//  correct.
static const double SWAPBLOCK = 512.0;

static bool vmstats( vm_statistics_data_t &vms ){
  return vm_statistics( task_self(), &vms ) == KERN_SUCCESS;
}

bool OSF1Stats::load( double avg[3] ){
...
bool OSF1Stats::cpu( double ticks[5] ){
...
bool OSF1Stats::memory( double &total, double &cache, double &free ){
  ...
  total = ( (double)vms.free_count + vms.active_count + vms.inactive_count
            + vms.wire_count ) * pagesize;
  cache = (double)vms.inactive_count * pagesize;
  free = (double)vms.free_count * pagesize;

  return true;
}

bool OSF1Stats::swap( double &total, double &free ){
  struct swapinfo swi;
  int i;
  ...
  for ( i = 0 ; table( TBL_SWAPINFO, i, &swi, 1, sizeof(swi) ) > 0 ; i++ ){
    total += (double)swi.si_swapsize * SWAPBLOCK;
    free += (double)swi.si_free * SWAPBLOCK;
  }

  return i > 0;
}

bool OSF1Stats::paging( double &in, double &out ){
  vm_statistics_data_t vms;
