...
MemMeter::MemMeter( XOSView *parent )
  ...
  ok_ = OSF1Stats::memory( total, cache, free );
...
void MemMeter::getmeminfo( void ){
  //  The inactive queue is where the buffer cache parks clean file pages; the
  //  kernel reclaims them on demand, so show them apart from used memory.
  double cache, free;

  if ( !OSF1Stats::memory( total_, cache, free ) )
    return;
