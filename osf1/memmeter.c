...
static void getmeminfo(MemMeter *mm) {
  /*  The inactive queue is where the buffer cache parks clean file pages;
   *  the kernel reclaims them on demand, so show them apart from used
   *  memory.  */
  FieldMeter *fm = &mm->f;
  double cache, freemem;

  if (!osf1stats_memory(&fm->total, &cache, &freemem))
    return;
...
Meter *memmeter_new(XOSView *parent) {
  ...
  mm->ok = osf1stats_memory(&total, &cache, &freemem);
