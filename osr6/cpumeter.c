...
/*  The MAS counters are registered per processor and summed by osr6stats, so
 *  a multiprocessor machine gets a single aggregate meter and the cpuFormat
...
static void getcputime(CPUMeter *cm) {
  ...
  if (!osr6stats_cpu(cm->cputime[cm->cpuindex]))
    return;
