...
/*  swapctl(2) totals every swap area, in 512 byte blocks, and needs no
 *  privileges.  */

#define SWAP_BLOCK 512

static int swapinfo(double *totalp, double *freep) {
    off_t total = 0, freeblks = 0;

    if (swapctl(SC_GETSWAPTOT, &total) < 0
        || swapctl(SC_GETFREESWAP, &freeblks) < 0)
        return 0;

    *totalp = (double)total * SWAP_BLOCK;
    *freep = (double)freeblks * SWAP_BLOCK;
...
static void getswapinfo(SwapMeter *sm) {
    ...
    if (!swapinfo(&fm->total, &fm->fields[1]))
        return;
