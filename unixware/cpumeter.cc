...
//  The MAS counters are registered per processor and summed here, so a
//  multiprocessor machine gets a single aggregate meter and the cpuFormat
...
void CPUMeter::getcputime( void ){
  if ( !UnixWareStats::cpu( cputime_[cpuindex_] ) )
    return;
