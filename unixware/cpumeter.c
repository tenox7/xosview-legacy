...
/*  The MAS counters are registered per processor and summed by
 *  unixwarestats, so a multiprocessor machine gets a single aggregate meter
 *  and the cpuFormat resource has no effect.  */
...
static void getcputime(CPUMeter *cm) {
  ...
  if (!unixwarestats_cpu(cm->cputime[cm->cpuindex]))
    return;
