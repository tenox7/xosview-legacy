...
The meters read everything through the interface in aix/aixstats.h,
...
top(1) uses, and is specific to 4.x -- the symbol still exists on 5.1
but its fields have moved.

/dev/kmem is mode 0440 root:system, so on 4.x xosview must run as root
or be installed setgid system.  Without that access every meter shows as
disabled.  5.x needs no privileges.
...
backwards is skipped rather than plotted as a spike.
